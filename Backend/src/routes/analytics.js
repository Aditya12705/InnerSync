...
router.post('/assessments/submit', protectRoute, async (req, res) => {
  try {
    ...
    if (student) {
      const subject = `New Assessment Submitted - ${student.name} (${type.toUpperCase()})`;
      const htmlContent = `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <h2 style="color: #2c3e50;">New Assessment Submitted</h2>
          <p><strong>${student.name}</strong> has successfully submitted a new mental health assessment.</p>
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
            <tr style="background: #f8f9fa; border-bottom: 1px solid #ddd;">
              <td style="padding: 10px; font-weight: bold;">Student:</td>
              <td style="padding: 10px;">${student.name} (${student.email})</td>
            </tr>
            <tr style="border-bottom: 1px solid #ddd;">
              <td style="padding: 10px; font-weight: bold;">Test Type:</td>
              <td style="padding: 10px;">${type.toUpperCase()}</td>
            </tr>
            <tr style="background: #f8f9fa; border-bottom: 1px solid #ddd;">
              <td style="padding: 10px; font-weight: bold;">Total Score:</td>
              <td style="padding: 10px;">${totalScore}</td>
            </tr>
            <tr style="border-bottom: 1px solid #ddd;">
              <td style="padding: 10px; font-weight: bold;">Severity:</td>
              <td style="padding: 10px;"><span style="text-transform: capitalize;">${severity}</span></td>
            </tr>
          </table>
        </div>
      `;
